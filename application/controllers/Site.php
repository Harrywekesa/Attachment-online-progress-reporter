            ...
            if (isset($login_details) && !empty($login_details)) {
                ...
                if ($user->is_active == "yes") {
                    ...
                    if ($result != false) {
                        ...
                        $sessionId = $this->db->select('session_id')->from('student_session')->where('student_id', $result[0]->id)->order_by('id', 'desc')->limit(1)->get()->row();
//                        $studentSession = $this->db->select('s.*, u.session_id')->from('student_session u')->join('sessions s', 'u.session_id = s.id')->where('u.student_id', $result[0]->id)->get()->result_array();
//
                        $session_data = array(
                            'id'              => $result[0]->id,
