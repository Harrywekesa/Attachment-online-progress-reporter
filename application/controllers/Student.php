...
class Student extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array("timeline_model", "student_edit_field_model", "attachment_model"));
    ...
    public function view($id)
    {
        ...
        $session                = $this->db->select('session_id')->from('student_session')->where('student_id', $id)->get()->row()->session_id;
    ...
    public function insurance_delete($id, $student_id)
    {
        $this->student_model->insurance_delete($id);
        $this->session->set_flashdata('msg', '<i class="fa fa-check-square-o" aria-hidden="true"></i> ' . $this->lang->line('delete_message') . '');
        redirect('student/view/' . $student_id);
    }
...
//                    $this->student_model->add($update_student);
                ...
                $student_login_detail = array('id' => $insert_id, 'credential_for' => 'student', 'username' => $this->input->post('admission_no'), 'password' => $user_password, 'contact_no' => $this->input->post('mobileno'), 'email' => $this->input->post('email'));
    ...
    public function create_insurance()
    {
        ...
        $this->form_validation->set_rules('company_name', $this->lang->line('company'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('first_doc', $this->lang->line('document'), 'callback_handle_uploadcreate_doc');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($_FILES["first_doc"]) && !empty($_FILES['first_doc']['name'])) {
                ...
                $policy = $this->input->post('policy_name');
                ...
                $img_name    = $uploaddir . basename($imp);
                move_uploaded_file($_FILES["first_doc"]["tmp_name"], $img_name);
                $data_img = array('student_id' => $student_id, 'insurance_company' => $company_name,'policy' =>$policy,'policy_number'=>$policy_number,'expires_on'=>$expires_on, 'insurance_image' => $imp, 'status'=>1);
                $this->student_model->addInsurance($data_img);
                $data_img1 = array('student_id' => $student_id, 'title' => 'Insurance Policy', 'doc' => $imp);
                $this->student_model->adddoc($data_img1);
    ...
    public function import()
    {
        ...
        $fields = array('admission_no','firstname', 'middlename', 'lastname', 'gender', 'dob', 'mobileno', 'email', 'admission_date');
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                           ...
                           $insert_id = $this->student_model->add($student_data[$i], $data_setting);
                            ...
                            if (!empty($insert_id)) {
                                ...
                                $this->student_model->add($update_student);
                                $sender_details = array('student_id' => $insert_id, 'contact_no' => $email, 'email' => $mobile_no);
                                $this->mailsmsconf->mailsms('student_admission', $sender_details);
            ...
            if (isset($search)) {
                if ($search == 'search_filter') {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $data['resultlist']  = $resultlist;
                        $title               = $this->classsection_model->getDetailbyClassSection($data['class_id'], $data['section_id'], $session_id);
                        $class1 = ($title['class'] != null)?$title['class']:'Undefined';
                        $section1 = ($title['section'] != null)?$title['section']:' undefined';
                        $data['title']       = 'Student Details for ' . $class1 . "(" . $section1 . ")";
                    }
