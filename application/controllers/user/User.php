...
//            $student_session_id = $this->input->post('clschg');
            ...
            $student        = $this->student_model->getByStudentSession('');
...
//        $student_current_class = $this->customlib->getStudentCurrentClsSection();
        $student_current_class = $this->db->select('class_id, section_id, session_id as student_session_id, id')->from('student_session')->where('student_id', $student_id)->limit(1)->get()->row();
        $current_class = $student_current_class->class_id;
        $current_section = $student_current_class->section_id;
//        if(empty($student_current_class)){
//            $current_class = 5;
//            $current_section = 1;
//        }
        $student = $this->student_model->getStudentByClassSectionID($current_class, $current_section, $student_id);

        $data = array();

        $data['student_one'] = $student;
        $data['class_id_one'] = $student_current_class->class_id;
        $data['section_id_one'] = $student_current_class->section_id;
        $data['student_id_one'] = $student_id;


        if (!empty($student)) {

            $student_session_id           = $this->session->userdata('session');
...
//            $data['gradeList']            = $gradeList;
            $data['session_name']           = $this->db->select('*')->from('sessions')->where('id', $student_session_id)->get()->row();
            $data['student']              = $student;
            $data['insurance']            = $this->student_model->get_insurance($student_id);
            $data['attachment_record']    = $this->attachment_model->get($student_id);

        } 
 
        $unread_notifications = $this->notification_model->getUnreadStudentNotification(11);
        ...
        $data['user_logged_id'] = $student_id;
    ...
    public function create_attachment_firm()
        {

            $this->form_validation->set_rules('company_name', 'Company/Institution Name Required', 'trim|required|xss_clean');
...
//                    'first_doc'   => form_error('first_doc'),
                ...
                $sessionId = $this->input->post('set_session_id');
                $checkAttachment = $this->db->select('*')->from('student_attachment')->where('session_id', $sessionId)->get()->row();
                if(!empty($checkAttachment)){

                    $array = array('status' => 'fail', 'message' => 'Another Attachment Session Exist');
                }
                else{
...
//                        $fileInfo = pathinfo($_FILES["first_doc"]["name"]);
                        ...
                        $sessionSetId = $this->input->post('set_session_id');
...
//                        move_uploaded_file($_FILES["first_doc"]["tmp_name"], $img_name);
                        $data_img = array(
                            ...
                            'company_address' => $physical_address,
                            'company_phone' => $phone_number,
                            'city' => $city,
                            'session_id' => $sessionSetId,
                            'county' => $county,
                        ...
                        $this->user_model->addCompany($data_img);
