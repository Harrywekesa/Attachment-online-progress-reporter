...
//            $session =  $this->db->select('session_id, session')->from('student_session')->join('sessions', 'student_session.session_id = sessions.id')->where('student_id', $student_data['student_id'])->order_by('student_session.id', 'desc')->limit(1)->get()->row();
...
//        }
    ...
    public function getCurrentSessionNameData() {
            $session_result = $this->get();
            return $session_result[0];
    }
   public function getCurrentAttachmentSessionID() {
       $session_result = $this->get();
       return $session_result[0]['session_id'];
    }
    ...
    public function getCurrentSetStudentSession(){

        $student_session = $this->session->userdata('session_array');
        $studentSession = json_encode($student_session);
