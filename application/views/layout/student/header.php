...
if($this->studentmodule_lib->hasActive('multi_class')){
 ...
 ?>
      ...
      <a data-toggle="modal" data-target="#user_sessionModal"><span>
              <?php echo $this->lang->line('current_session') . ": " . $this->setting_model->getCurrentSessionName(); ?>
              <?php
//              $sessionId = $this->db->select('session_id')->from('student_session')->where('student_id', $student_data['student_id'])->order_by('id', 'desc')->limit(1)->get()->row();
                //$sessions = $this->db->select('session_id, session')->from('student_session')->join('sessions', 'student_session.session_id = sessions.id')->where('student_id', $student_data['student_id'])->order_by('student_session.id', 'desc')->limit(1)->get()->row();
             // echo $sessions->session;  ?>
          </span><i class="fa fa-pencil pull-right"></i>
      </a>
                    ...
                    <ul class="sidebar-menu verttop38" id="sibe-box">
                       ...
                       if($this->module_lib->hasModule('zoom_live_classes') && $this->studentmodule_lib->hasActive('live_classes') && $this->auth->addonchk('sszlc',false)){                     
                            ?>
 <li class="<?php echo set_topmenu('Conference'); ?>"><a href="<?php echo base_url('user/conference'); ?>"><i class="fa fa-video-camera ftlayer"></i>
...
<!--                            <li class="--><?php //echo set_Topmenu('Time_table'); ?><!--"><a href="--><?php //echo base_url(); ?><!--user/timetable"><i class="fa fa-calendar-plus-o ftlayer"></i> <span>--><?php //echo $this->lang->line('class_timetable'); ?><!--</span></a></li>-->
                            <li class="<?php echo set_Topmenu('Elog_book'); ?>"><a href="<?php echo base_url(); ?>user/elogbook"><i class="fa fa-calendar-plus-o ftlayer"></i> <span><?php echo $this->lang->line('class_elogbook'); ?></span></a></li>
                            <?php
                        }
